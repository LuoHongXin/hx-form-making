<template>
  <div class="combination-form-item mk-form-item">
    ...
    <y-form-model-item
      :class="formItemData.options.inline === false ? 'flex-form-item vertical-mode' : 'flex-form-item'"
      :rules="{
    ...
    >
      ...
      >
        <MkFormItem
          ...
          :class="
            (operateFormItem && operateFormItem.key === item.key ? 'selected ' : '') +
            (formItemData.options.showChildComponentLabel ? 'showLabel' : '')
          "
          :formItemData="item"
    ...
    handleItem(obj) {
      ...
      for (let index = 0; index < initFormData.length; index++) {
        ...
        if (element.type === obj.type) {
          ...
          element.parentType = 'combination'; // 表格内组件添加标识
          const newItems = {
            ...element,
